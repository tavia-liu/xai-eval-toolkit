...
def summarise_traits(participant: pd.Series, include_all: bool = False) -> str:
    """
    Construct a persona description from a participant's survey responses.

    Parameters
    ----------
    participant : pandas.Series
        The row corresponding to a single participant from the survey CSV.
    include_all : bool, optional
        If True, include all survey questions and answers (except
        identifiers and administrative columns).  If False (default),
        include only a curated subset of key questions.

    Returns
    -------
    str
        A semicolon‑separated string summarising the participant's traits.

    Notes
    -----
    Including the full survey may provide the model with more context
    about the participant's domain knowledge, attitudes and preferences.
    However, it will also make the prompt longer and may increase
    token usage.  Choose the appropriate setting based on model
    capabilities and desired fidelity.
    """
    if include_all:
        # Exclude sensitive or irrelevant columns from the full summary
        excluded = {
            "candidate_id",
            "Start time",
            "End time",
        ...
        parts: List[str] = []
        for col, val in participant.items():
            if col in excluded:
                continue
        ...
        # Curated summary focusing on a few key questions
        cols = {
...
def main(args: argparse.Namespace) -> None:
    ...
    for _, participant in survey_df.iterrows():
        ...
        traits = summarise_traits(participant, include_all=args.include_full_survey)
    ...
    if total > 0:
        ...
        true_certs = merged["participant_certainty_en_true"].str.lower().str.strip()
        sim_certs = merged["participant_certainty_en_sim"].str.lower().str.strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--include_full_survey",
        action="store_true",
        help="Include all survey answers (except identifiers) in the participant persona instead of a concise summary",
    )
